...
# TODO: replace with ZoneInfo when using 3.9+
...
async def future_time_from_interaction(argument: str, interaction: discord.Interaction) -> tuple[str, datetime.datetime]:
    ...
    if reminder is not None:
        ...
        if timezone is not None:
            tzinfo = gettz(timezone) or datetime.UTC
        else:
            timezone = 'UTC'
...
def state_emoji(opt: bool | None) -> str:
    lookup = {
        True: '<:Check:1368150350741377116>',
        False: '<:Cross:1368150352221700137>',
        None: '<a:loading:1400479061104660541>',
    }
    return lookup[opt]
...
class TodoItem:
    ...
    def __init__(self, cog: Todo, record: Any) -> None:
        ...
        self.timezone = record.get('timezone', 'UTC')
...
class AddTodoModal(ui.Modal, title='Add Todo'):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.followup.send(
            content=f'<:Check:1368150350741377116> Added todo item {item.id}.',
            embed=item.embed,
            ephemeral=True,
        )
...
class TodoPages(RoboPages):
    ...
    async def delete_todo(self, interaction: discord.Interaction, button: ui.Button):
        assert interaction.message is not None
        confirm = ConfirmationView(timeout=60.0, author_id=interaction.user.id, delete_after=True, disable=False)
...
class Todo(commands.Cog):  # noqa: PLR0904
    """Manage a todo list."""

    def __init__(self, bot: RoboDan) -> None:
    ...
    async def get_readable_channel(
        self, channel_id: int, guild_id: int | None, user: discord.abc.User
    ...
    @tasks.loop(time=datetime.time(0, 0, 0, tzinfo=datetime.UTC))
    ...
    async def add_todo(
        ...
        *,
        ...
        if due_date is not None and due_date.tzinfo is not None:
            due_date = due_date.astimezone(datetime.UTC).replace(tzinfo=None)
    ...
    async def todo_add(self, ctx: Context, *, content: str | None = None) -> None:
        ...
        await ctx.send(content=f'<:Check:1368150350741377116> Added todo item {item.id}.', view=view, embed=item.embed)
    ...
    async def todo_add_slash(
        ...
        content: str,
        due_date: app_commands.Transform[datetime.datetime, time.TimeTransformer] | None = None,
    ):
        """Adds a todo item
